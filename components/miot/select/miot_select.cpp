...
void MiotSelect::setup() {
  this->parent_->register_listener(
      this->siid_, this->piid_, this->poll_, mvtUInt,
      [this](const MiotValue &value) {
        ESP_LOGV(TAG, "MCU reported select %" PRIu32 ":%" PRIu32
                      " is: %" PRIu32,
                 this->siid_, this->piid_, value.as_uint);

        // FIX: bind options by reference (no copy)
        const auto &options = this->traits.get_options();

        // mappings_ is fine as-is
...
void MiotSelect::control(const std::string &value) {
  ...
  ESP_LOGV(TAG,
           "Setting select %" PRIu32 ":%" PRIu32 " value to %" PRIu32 ":%s",
           this->siid_, this->piid_, mapping, value.c_str());
...
void MiotSelect::dump_config() {
  ...
  // FIX: bind by reference
  const auto &options = this->traits.get_options();

  for (size_t i = 0; i < this->mappings_.size(); i++) {
    // FIX: options.at(i) is already const char*, no .c_str()
    ESP_LOGCONFIG(TAG, "    %i: %s",
                  this->mappings_.at(i),
                  options.at(i));
  }
}
